...
| 질문 | 답변 | 판단 방향 | 기술적 근거 |
|---|---|---|---|
| AI Agent가 처리하는 데이터를 외부 클라우드로 전송할 수 있는가? | **전면 허용** | → 2단계로 이동 | 상용 LLM API 또는 클라우드 GPU 인스턴스 기반 셀프호스팅 모두 선택 가능 (→ 3장 LLM 선정 분기 1) |
...
- **데이터 처리 투명성**: 상용 API의 로깅·학습 정책에 대한 우려 해소 (전면 금지는 아니지만 선호하지 않는 경우)
