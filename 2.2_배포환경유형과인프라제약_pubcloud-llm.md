...
## 2.2.5 최소 인프라 요구사항 기준선

환경 유형이 확정되면, 3장~11장 진입 전에 해당 환경에서 AI Agent 시스템을 운영하기 위한 **최소 인프라**를 확인해야 한다(SHOULD). 아래 표는 환경 유형별로 확보해야 하는 핵심 인프라를 요약한 것이며, 누락된 항목은 프로젝트 착수 이후 병목이 되므로 사전에 점검한다.

| 환경 유형 | 최소 인프라 요구사항 |
|---|---|
| **Public Cloud (상용 API 경로)** | LLM API 키·Provider 계약, 컨테이너 런타임(Docker/K8s) 또는 서버리스, 관리형 DB 접근(RDS, Cosmos DB 등), 네트워크(VPC 내 통신 또는 인터넷) |
| **Public Cloud (셀프호스팅 경로)** | GPU 인스턴스(EKS/AKS/GKE GPU 노드풀), LLM 서빙 프레임워크(vLLM/TGI), 모델 가중치 저장소(S3/GCS), 관리형 DB, VPC 내부 GPU↔Agent 저지연 네트워크 |
| **Private Cloud (K8s)** | K8s/OpenShift 클러스터, GPU 서버 + NVIDIA GPU Operator, LLM 서빙 프레임워크(vLLM/TGI), 셀프호스팅 DB(PostgreSQL, Redis, Vector DB), 클러스터 내부 네트워크(Ingress/Service Mesh) |
| **On-Premise** | GPU 서버(LLM 서빙용), LLM 서빙 프레임워크(vLLM/TGI/Ollama), CPU 서버(Agent/MCP 실행), 스토리지(Vector DB, 로그, 모델 가중치), 네트워크(GPU↔Agent 간 저지연), 폐쇄망 구성(해당 시) |
| **Hybrid** | 상용 API 경로 또는 Public Cloud 셀프호스팅 요구사항 + Private Cloud/On-Premise 요구사항 + 환경 간 보안 네트워크(VPN/전용선/PrivateLink) |

> **참고**: 위 표는 프로젝트 착수 시 "무엇을 확보해야 하는가"를 점검하기 위한 체크리스트이다. 각 항목의 구체적인 사양·제품·구성 방법은 2.2.7절 참조 문서를 활용한다.

### 셀프호스팅 시 GPU 인프라 규모 산정 원칙

셀프호스팅 경로를 선택한 경우(Private Cloud / On-Premise / Public Cloud GPU 인스턴스 / Hybrid의 내부 경로), 모델 크기에 따라 필요한 GPU 인프라 규모가 크게 달라진다. 아래 원칙에 따라 규모를 산정해야 한다(SHOULD).

1. **모델 파라미터 규모가 GPU VRAM 요구량을 결정한다.** 소형(~8B)에서 초대형(70B+)까지 필요 VRAM이 수 배~수십 배 차이가 나며, GPU 종류·수량·비용에 직접 영향을 미친다.
...
3. **MoE 모델은 총 파라미터 수와 Active 파라미터 수가 다르다.** 전체 Expert 가중치를 로드해야 하므로, Dense 모델 대비 VRAM 산정 방식이 달라진다.
...
> **상세 참조**: 모델 크기별 VRAM 산정표, 클라우드 인스턴스 매핑(AWS/Azure/GCP), On-Premise 서버 구성, GPU 공유 전략(MIG/Time-Slicing), 양자화 방식별 품질·성능 비교 등은 `멀티환경 Kubernetes·GPU 인프라 정의서`를 참조한다.
...
배포 환경은 2.3절 의존 관계 맵에서 **가장 상위의 제약**으로 작용하며, 아래와 같은 연쇄 영향 체인을 발생시킨다.
...
| **셀프호스팅 시 인프라 규모 산정 원칙 (부록)** | 2.2.5절 GPU 규모 산정 원칙의 보충 해설, 원칙별 적용 예시 | 셀프호스팅 경로에서 GPU 규모 산정 시 2.2.5절과 함께 참조 |
