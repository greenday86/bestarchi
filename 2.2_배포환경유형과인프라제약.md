...
> **Private Cloud를 별도 유형으로 분리한 근거**: 컨테이너 오케스트레이션 플랫폼의 유무는 GPU 스케줄링, 선언적 배포, 오토스케일링 가능 여부를 결정하며, 이는 3장~11장의 기술 선택지를 근본적으로 분기시킨다. 동일하게 "데이터 반출 불가"인 환경이라도 K8s 기반(Private Cloud)과 베어메탈(On-Premise)은 운영 복잡도·확장성·기술 후보군에서 큰 차이를 보이므로, 의사결정 정밀도를 위해 분리한다.
...
flowchart TD
    ...
    Q1 -->|"전면 가능"| Q2{"2단계\n자체 GPU 인프라를\n보유/투자 계획이 있는가?"}
    Q1 -->|"전면 불가"| Q3{"3단계\nK8s/OpenShift\n도입 가능한가?"}
    Q1 -->|"일부 가능"| Hybrid["Hybrid 방향 검토\n민감→Private/OnPrem\n비민감→Public Cloud"]

    Q2 -->|"없음"| PublicCloud["Public Cloud\n우선 검토"]
    Q2 -->|"있음"| Compare["Public Cloud 또는\nHybrid 비교 검토"]

    Q3 -->|"예"| PrivateCloud["Private Cloud\n우선 검토"]
    Q3 -->|"아니오"| OnPrem["On-Premise\n방향 검토"]

    PublicCloud --> Q4["4단계: SLA·확장성 검증"]
    Compare --> Q4
    PrivateCloud --> Q4
    OnPrem --> Q4
    Hybrid --> Q4

    Q4 --> Q5["5단계: 운영 역량·비용 구조 최종 검증"]
...
| 질문 | 답변 | 판단 방향 | 기술적 근거 |
|---|---|---|---|
| AI Agent가 처리하는 데이터를 외부 클라우드로 전송할 수 있는가? | **전면 가능** | → 2단계로 이동 | 상용 LLM API(GPT, Claude, Gemini)를 직접 호출할 수 있어 모델 서빙 인프라 구축 부담이 없음 (→ 3장 LLM 선정 분기 1) |
| | **전면 불가** | → Private Cloud 또는 On-Premise 방향, 3단계로 이동 | 개인정보보호법·금융위 클라우드 이용 가이드·ISMS-P·국방 보안규정 등에 의해 외부 전송이 금지되는 경우, **오픈소스 LLM 셀프호스팅이 필수**(MUST)이며 GPU 인프라를 내부에 확보해야 함 (→ 3장 분기 2-A 온프레미스 경로) |
| | **일부 가능** | → Hybrid 방향, 4단계로 이동 | 데이터 등급(공개/내부/기밀)에 따라 비민감 질의는 상용 API로, 민감 질의는 내부 모델로 분기하는 **라우팅 레이어 설계**가 필요 (→ 3장 분기 1 하이브리드 라우팅) |

**하이브리드 라우팅 설계 포인트** (1단계에서 "일부 가능"으로 진입한 경우)
...
배포 환경 판단 시 데이터 반출 가능 여부와 함께 **연결 격리 수준**을 동시에 정의해야 한다(SHOULD). 연결 격리 수준은 `L1 Internet`, `L2 Private Endpoint`, `L3 Dedicated Line`, `L4 Air-gapped`로 구분하고, 각 수준별 허용 LLM 호출 경로와 egress 정책을 명시한다. Hybrid 환경에서는 등급별 라우팅 규칙과 연결 격리 수준을 일관되게 설계한다.
...
| 질문 | 답변 | 판단 방향 | 기술적 근거 |
|---|---|---|---|
| 자체 데이터센터·GPU 인프라를 보유하고 있거나 신규 투자 계획이 있는가? | **없음** | → **Public Cloud** 우선 검토 | Bedrock·Azure OpenAI·Vertex AI 등 관리형 서비스를 사용하면 GPU 프로비저닝·모델 배포·오토스케일링을 사업자가 처리하므로 Time-to-Market이 가장 짧음 (→ 3장 분기 2-B 상용 API 경로) |
| | **있음** | → Public Cloud 또는 Hybrid 비교 검토 | A100/H100 등 고가 GPU를 이미 보유한 경우, LLM 추론 워크로드를 내부에서 처리하면 토큰당 비용이 상용 API 대비 절감될 수 있음. 다만 모델 업데이트·서빙 파이프라인·장애 대응 등 운영 오버헤드를 함께 비교해야 함 (→ 3.4절 모델 조합 전략) |

### 3단계 — 컨테이너 플랫폼 유무 (데이터 반출 불가 시)

데이터 반출이 불가한 환경에서 AI Agent를 운영하려면, 모델 서빙·Vector DB·Observability 등 여러 컴포넌트를 내부에 직접 배포해야 한다. 이때 **컨테이너 오케스트레이션 플랫폼의 유무가 운영 복잡도와 확장성을 결정짓는 핵심 분기점**이 된다.
...
| 질문 | 답변 | 고려사항 |
|---|---|---|
| K8s/OpenShift 운영 경험과 GPU 관리 인력이 확보되어 있는가? | **미확보** | Private Cloud·On-Premise를 선택할 경우 GPU 드라이버·CUDA 버전 관리, 모델 양자화 튜닝, 서빙 파이프라인 장애 대응 등의 운영 부담이 크므로, Public Cloud로 재검토하거나 외부 운영 위탁(Managed Service)을 함께 고려 |
| | **확보** | Private Cloud·On-Premise 운영 가능. 다만 모델 업데이트 주기, GPU 노드 증설 계획, 클러스터 업그레이드 전략 등 중장기 운영 로드맵을 사전에 수립해야 함(SHOULD) |
...
LLM 후보가 정해지면 **추론 서빙 구조**(예: vLLM/TGI/Triton, 실시간/배치, 단일/다중 모델)를 환경 제약에 맞춰 확정해야 한다(SHOULD). Public Cloud는 관리형 추론 엔드포인트를 우선 검토하고, Private Cloud/On-Premise는 GPU 스케줄링·확장 전략을 포함한 셀프호스팅 서빙 구조를 정의한다. 서빙 구조 결정 결과는 4장 Framework 및 11장 Observability 설계 입력으로 사용한다.

### Public Cloud 환경 (AWS, Azure, GCP)

**특성**: 관리형 서비스 활용 가능, 자동 스케일링 지원, 초기 투자 없이 종량제(OpEx) 사용.

**기술 선정 영향**:
- **LLM(3장)**: 상용 LLM API 즉시 사용 가능 — AWS Bedrock, Azure OpenAI, GCP Vertex AI. 3.2절 의사결정 트리 분기 2-B(상용 API 경로)로 진입.
- **Framework(4장)**: 모든 Framework가 상용 LLM Provider를 네이티브 지원하므로 제약 없음.
- **Data Store(8장)**: 관리형 Vector DB 활용 가능 — Aurora pgvector, Azure AI Search, Vertex AI Vector Search, Pinecone.
- **Observability(11장)**: SaaS 도구(LangSmith, Helicone) 즉시 연동 가능.

**고려사항**: 데이터가 외부 클라우드로 전송되므로 데이터 주권(Data Sovereignty) 및 규제 요건 확인 필수(MUST).

**적합 시나리오**: PoC/시연, 글로벌 서비스, 탄력적 트래픽 대응이 필요한 서비스.
...
**적합 시나리오**: 금융·공공·국방·의료 등 데이터 반출 불가 환경, 이미 OpenShift/K8s를 운영 중인 조직, 폐쇄망에서 Cloud-like 운영을 원하는 경우.
...
**기술 선정 영향**:
- **LLM(3장)**: 데이터 등급 기반 LLM 호출 분기 — 민감 질의는 내부 오픈소스 모델, 비민감 질의는 상용 API. 3.4절 Model Routing과 연계 설계.
...
|---|---|---|---|---|
| **LLM (3장)** | 상용 API (GPT, Claude, Gemini) — 분기 2-B | 오픈소스 셀프호스팅 (vLLM/TGI on K8s) — 분기 2-A | 오픈소스 셀프호스팅 + GPU 서버 직접 관리 | 민감 → 내부 모델 / 비민감 → 상용 API — 분기 1 하이브리드 |
...
| **Tool 연동 (5장)** | FC 기본, MCP 자유 선택 | MCP 서버를 K8s Pod로 배포 — 중앙 관리 용이 | FC 단독 우선, MCP는 인프라 부담 검토 후 | 환경별 MCP 서버 분리 또는 통합 게이트웨이 |
...
| **Embedding (8장)** | 상용 API (OpenAI, Cohere, Gemini) | 오픈소스 셀프호스팅 (BGE-M3, Qwen3-Embedding) | 오픈소스 셀프호스팅 | 데이터 등급에 따라 선택 |
...
| **Observability (11장)** | SaaS (LangSmith, Helicone) — 분기 A/C | K8s 내장 모니터링 + Langfuse 셀프호스팅 — 분기 B | Langfuse/Arize Phoenix 셀프호스팅 — 분기 B | 비식별화 후 외부 전송 또는 환경별 분리 |

---

## 2.2.5 최소 인프라 기준선

오픈소스 LLM을 셀프호스팅하는 환경(Private Cloud / On-Premise / Hybrid의 내부 경로)에서는 모델 크기에 따라 최소 GPU 인프라 요구사항이 달라진다. 아래는 요약 수준의 기준선이며, 환경별·인스턴스별 상세 스펙은 참조 문서를 확인한다.

| 모델 크기 | 파라미터 규모 | 필요 VRAM (4-bit 양자화 기준) | 대표 GPU 구성 | 대표 유스케이스 |
|---|---|---|---|---|
| 소형 | 7B ~ 8B | 8 ~ 12 GB | A10/L4/RTX 4090 ×1 | 개인 비서, 간단한 요약 |
| 중형 | 13B ~ 14B | 16 ~ 20 GB | A10/RTX 4090 ×1 | RAG 챗봇 |
| 대형 | 30B ~ 34B | 24 ~ 40 GB | A100 40GB ×1 또는 A10 ×2 | 분석, Reasoning |
| 초대형 | 70B 이상 | 45 ~ 80 GB | A100 80GB ×1 또는 H100 이상 | 기업 AI Assistant |

> **상세 참조**: 클라우드 환경 인스턴스 매핑(AWS/Azure), On-Premise 서버 구성(RAM/CPU), GPU 공유 전략(MIG/Time-Slicing) 등은 `멀티환경 Kubernetes·GPU 인프라 정의서`를 참조한다.

Public Cloud 환경에서 상용 LLM API만 사용하는 경우, 위 GPU 인프라는 불필요하다. 다만 자체 Embedding 모델 서빙이나 Guardrails 모델(LlamaGuard 등) 운영 시에는 소형~중형 수준의 GPU가 필요할 수 있다.
...
1. **배포 환경 → LLM → Framework**: 온프레미스를 선택하면 오픈소스 LLM으로 제한되고, 이는 Framework의 LLM Provider 호환성 기준(4.4.3절)을 통해 Framework 후보를 좁힌다.
...
| **멀티환경 Kubernetes·GPU 인프라 정의서** | 네트워크 설계(VPC/서브넷/egress), 스토리지(CSI/PV), GPU 노드 구성(드라이버/Operator/MIG), 오토스케일링, 보안·운영·거버넌스, IaC 예시 | 인프라 설계·구성·운영 표준을 수립할 때 |
