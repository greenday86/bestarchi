...
| 질문 | 답변 | 판단 방향 | 기술적 근거 |
|---|---|---|---|
| AI Agent가 처리하는 데이터를 외부 클라우드로 전송할 수 있는가? | **전면 허용** | → 2단계로 이동 | 상용 LLM API(GPT, Claude, Gemini)를 직접 호출할 수 있어 모델 서빙 인프라 구축 부담이 없음 (→ 3장 LLM 선정 분기 1) |
| | **전면 금지** | → Private Cloud 또는 On-Premise 방향, 3단계로 이동 | 개인정보보호법·금융위 클라우드 이용 가이드·ISMS-P·국방 보안규정 등에 의해 외부 전송이 금지되는 경우, **오픈소스 LLM 셀프호스팅이 필수**(MUST)이며 GPU 인프라를 내부에 확보해야 함 (→ 3장 분기 2-A 온프레미스 경로) |
...
### 2단계 — 인프라 소유 및 운영 전략 (데이터 외부 전송 허용 시)
